...
object RLP {
    ...
    fun encodeBigInteger(srcBigInteger: BigInteger): ByteArray {
        ...
        return if (srcBigInteger == BigInteger.ZERO)
            ...
            encodeElement(srcBigInteger.asUnsignedByteArray())
    }
    ...
    fun decode(data: ByteArray, posParam: Int): DecodeResult {
        ...
        when {
            ...
            prefix < OFFSET_SHORT_LIST -> {
                ...
                return DecodeResult(
                    pos + 1 + lenlen + lenbytes, data.copyOfRange(
    ...
    private fun encodeLength(length: Int, offset: Int) = when {
        ...
        length < MAX_ITEM_LENGTH -> {
            ...
            byteArrayOf(firstByte) + binaryLength
        }
    ...
    private fun toBytes(input: Any?): ByteArray = when (input) {
        ...
        is Long -> if (input == 0) byteArrayOf() else (BigInteger.valueOf(input)
            .asUnsignedByteArray())
        is Int -> if (input == 0) byteArrayOf() else (BigInteger.valueOf(input.toLong())
            .asUnsignedByteArray())
        is BigInteger -> if (input == BigInteger.ZERO) byteArrayOf() else (input.asUnsignedByteArray())
    ...
    fun fullTraverse(
        ...
        if (level > MAX_DEPTH) {
            throw RuntimeException(
                String.format(
                    "Error: Traversing over max RLP depth (%s)",
                    MAX_DEPTH
                )
            )
        }
        ...
        try {
            ...
            while (pos < endPos) {
                ...
                if (msgData[pos].toInt() and 0xFF > OFFSET_LONG_LIST) {
                    ...
                    val lengthOfLength =
                        ((msgData[pos].toInt() and 0xFF) - OFFSET_LONG_LIST).toByte()
                    ...
                    System.arraycopy(
                        msgData, pos, rlpData, 0, lengthOfLength.toInt()
                                + length + 1
                    )
            ...
            throw RuntimeException(
                "RLP wrong encoding (" + msgData?.copyOfRange(
                    startPos,
                    endPos - startPos
                )?.toHex() + ")", e
            )
            ...
            throw RuntimeException(
                "Invalid RLP (excessive mem allocation while parsing) (" + msgData?.copyOfRange(
                    startPos,
                    endPos - startPos
                )?.toHex() + ")", e
            )
