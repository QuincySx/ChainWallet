package com.smallraw.chain.lib.bitcoin.transaction.script

import com.smallraw.chain.lib.bitcoin.utils.IntUtil
    ...
    fun push(data: ByteArray): ByteArray {
        val bytes = when (val length = data.size) {
            ...
            in 0x10000..0xffffffff -> {
                val lengthBytesInLE = IntUtil.intToByteArray(length).reversedArray()
