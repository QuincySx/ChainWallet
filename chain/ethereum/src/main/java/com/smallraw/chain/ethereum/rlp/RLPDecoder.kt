...
fun ByteArray.decodeRLP(offset: Int = 0) = decodeRLPWithSize(offset).element
...
private fun ByteArray.decodeRLPWithSize(offset: Int = 0): DecodeResult {
    ...
    return when {
        ...
        value < LIST_OFFSET -> (value - ELEMENT_OFFSET).let {
            ...
            DecodeResult(
                copyOfRange(
                    lengthAndOffset.offset,
                    lengthAndOffset.offset + lengthAndOffset.length
                ).toRLP(), lengthAndOffset.length + lengthAndOffset.offset - offset
            )
        }
