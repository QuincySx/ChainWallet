...
object TransactionSerializer {
    ...
    fun hashForWitnessSignature(
        ...
        transaction.copy().apply {
            ...
            if (!anyoneCanPay) {
                val bosHashPrevouts =
                    BitcoinOutputStream(40 * inputs.size)
                inputs.forEach {
            ...
            if (!anyoneCanPay && signAll) {
                val bosSequence =
                    BitcoinOutputStream(8 * inputs.size)
                inputs.forEach {
            ...
            if (signAll) {
                val bosHashOutputs =
                    BitcoinOutputStream(64 * outputs.size)
