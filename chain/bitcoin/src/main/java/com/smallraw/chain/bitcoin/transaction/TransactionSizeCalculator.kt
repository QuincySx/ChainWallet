...
class TransactionSizeCalculator {
    ...
    private val p2shwpkhLength = 22 + 1       // 0014<20-byte-script-hash> + pushByte
    private val p2shwshShLength = 34 + 1       // 0014<20-byte-script-hash> + pushByte

    private val legacyTx =
        16 + 4 + 4 + 16    // 40 Version + number of inputs + number of outputs + locktime
    ...
    private val witnessData =
        1 + signatureLength + pubKeyLength  // 108 Number of stack items for input + Size of stack item 0 + Stack item 0, signature + Size of stack item 1 + Stack item 1, pubkey

    private val sigLengths = mapOf(
        ScriptType.P2PKH to signatureLength + pubKeyLength,
        ScriptType.P2PK to signatureLength,
        ScriptType.P2SHWPKH to p2shwpkhLength,
        ScriptType.P2SHWSH to p2shwshShLength
    )
    ...
    private val lockingScriptSizes = mapOf(
        ...
        ScriptType.P2SHWPKH to 23,
        ScriptType.P2SHWPKH to 33
    )
    ...
    private fun inputSize(output: TransactionOutput): Int {
        val scriptSigLength = when (output.scriptType) {
            ...
            ScriptType.P2SHWPKH -> p2shwpkhLength
            ScriptType.P2SHWSH -> p2shwpkhLength
            ScriptType.P2SH -> {
                output.redeemScript?.let { redeemScript ->
                    ...
                    if (signatureScriptFunction != null) {
                        ...
                        signatureLength + pubKeyLength + Script(Chunk(redeemScript)).scriptBytes.size
                    }
                } ?: 0
            }
            else -> 0
    ...
    fun transactionSize(
        ...
        val inputWeight =
            previousOutputs.map { inputSize(it) * 4 + if (txIsWitness) witnessSize(it.scriptType) else 0 }
                .sum()
